...
add_ml_results <- function( ml_results = NULL, repo = NULL, prefix = NULL ) {
    ...
    ml_results_filename <- paste0(tempfile(pattern=paste0(prefix,'-'), tmpdir=paste0(workdir(myrepo),'/inst/results')),'.rds')
    ...
    # delete pre-existing pull request
    # is there a pre-existing pull request?
