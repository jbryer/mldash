...
Although there are utility functions to install `keras`, `tensorflow`, and `torch` from their respective packages, I found them to not always work as expected. The `conda_install` function will ensure the Python packages are installed into the correct environment.
...
Lastly, ensure that `reticulate` uses the correct Python by setting the `RETICULATE_PYTHON` environment variable.
...
```{r run_models, eval = TRUE, warning=FALSE, message=FALSE, error=FALSE}
